...
class JobResource[F[_]: Async] private (jobs: Jobs[F]) extends Http4sDsl[F] {
  ...
  private val get: HttpRoutes[F] = HttpRoutes.of[F] { case GET -> Root / UUIDVar(id) =>
    jobs.get(id).flatMap {
      case None =>
        logger.info(s"No job found with id: $id")
          >> NotFound(ErrorResponse.ResourceNotFound(s"No job found with id: $id"))
      case Some(job) =>
        logger.info(s"Found job with id: $id")
          >> Ok(job)
    }
  }
    ...
    req.validate[JobInfo].flatMap {
      ...
      case Valid(jobInfo) =>
        jobs.create("email@example.com", jobInfo).flatMap { id =>
          logger.info(s"Created job with id: $id")
            >> Created(id)
        }
    }
  }
    ...
    req.validate[JobInfo].flatMap {
      ...
      case Valid(jobInfo) =>
        jobs.update(id, jobInfo).flatMap { updated =>
          if updated then
            logger.info(s"Updated job with id: $id")
              >> NoContent()
          else
            logger.info(s"No job found with id: $id")
              >> NotFound(ErrorResponse.ResourceNotFound(s"No job found with id: $id"))
  ...
  private val delete: HttpRoutes[F] = HttpRoutes.of[F] { case DELETE -> Root / UUIDVar(id) =>
    jobs.delete(id).flatMap { deleted =>
      if deleted then
        logger.info(s"Deleted job with id: $id")
          >> NoContent()
      else
        logger.info(s"No job found with id: $id")
          >> NoContent()

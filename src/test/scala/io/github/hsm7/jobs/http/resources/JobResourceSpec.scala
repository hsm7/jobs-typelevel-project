...
class JobResourceSpec extends AsyncFreeSpec with AsyncIOSpec with Matchers with Http4sDsl[IO] with JobFixture {
  ...
  val jobsMock: Jobs[IO] = new Jobs[IO] {
    ...
    override def getAll(pagination: Pagination, filters: JobFilters): IO[List[Job]] =
      if filters.remote.isEmpty then IO.pure(List(SCALA_JOB))
      else if filters.remote.get then IO.pure(List.empty)
      else IO.pure(List(SCALA_JOB))
  ...
  "JobResource" - {
    "Should return a job with a given ID" in {
      ...
      for {
        ...
        job      <- response.as[Job]
      } yield (response.status, job) shouldBe (Status.Ok, SCALA_JOB)
    }
    ...
    "Should return all jobs" in {
      ...
      } yield (response.status, jobs) shouldBe (Status.Ok, List(SCALA_JOB))
    }

    "Should filter jobs by remote flag" in {
      val locationWithTrueRemote  = uri"/jobs?remote=true"
      val locationWithFalseRemote = uri"/jobs?remote=false"
      for {
        responseWithTrueRemote  <- jobRoutes.orNotFound.run(Request(method = Method.GET, uri = locationWithTrueRemote))
        responseWithFalseRemote <- jobRoutes.orNotFound.run(Request(method = Method.GET, uri = locationWithFalseRemote))
        remoteJobs              <- responseWithTrueRemote.as[List[Job]]
        nonRemoteJobs           <- responseWithFalseRemote.as[List[Job]]
      } yield (
        responseWithTrueRemote.status,
        responseWithFalseRemote.status,
        remoteJobs,
        nonRemoteJobs
      ) shouldBe (Status.Ok, Status.Ok, List.empty, List(SCALA_JOB))
    }
    ...
    "Should create new job" in {
      ...
      } yield (response.status, uuid) shouldBe (Status.Created, NEW_JOB_UUID)
    }
